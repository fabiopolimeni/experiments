...
bool graphics::texture::create(const std::string & filename)
{
	m_TextureName = 0;

	if (!filename.empty())
		m_TextureName = build(filename.c_str());

	return m_TextureName != texture::invalid;
}

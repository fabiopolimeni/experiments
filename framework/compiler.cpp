...
bool compiler::checkShader(GLuint const & Name) const
{
	...
	files_map::const_iterator ShaderIterator = ShaderFiles.find(Name);
	if (ShaderIterator != ShaderFiles.end())
	{
		GLuint ShaderName = Name;
		GLint Result = GL_FALSE;
		...
		if (InfoLogLength > 0)
		{
			...
			fprintf(stdout, "Shader %s errors:\n%s\n", ShaderIterator->second.c_str(), &Buffer[0]);
		}
